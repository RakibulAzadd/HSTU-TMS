...
function getAllTransportSpecialTown($conn){
...
// DELETE special town to campus
function removeTransportSpecialTown($id, $conn){
  $sql  = "DELETE FROM t_special_town
          WHERE ID=?";
  $stmt = $conn->prepare($sql);
  $re   = $stmt->execute([$id]);
  if ($re) {
