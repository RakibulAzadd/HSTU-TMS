...
if(isset($_POST['user_name'])&& 
  ...
  //  $data = "fname=".$fname."&uname=".$uname;
    ...
    if (empty($name)) {
    	...
    	$sql = "INSERT INTO requisition(user_name,name, title_name, d_name,f_name,email_address,phone_number,
        j_date,j_start,j_end,p_use,destination,transport_type,transport_no,driver_name ,status  ) 
    	        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    	$stmt = $conn->prepare($sql);
    	$stmt->execute([$uname,$name, $title_name,$department,$faculty,$email,$phone,$date,$start,$end,$cause,$destination,$transport_type,
