...
if (isset($_POST['t_name']) &&
    ...
    include "../transport-add-special-town.php";
 ...
 // include('')
    if (empty($t_name)) {
		...
		header("Location: ../transport-add-special-town.php?error=$em");
		exit;
	}else if (empty($day)) {
        ...
        $sql  = "INSERT INTO
                 t_special_town(t_name, time, from_d, to_d, t_no,day)
                 VALUES(?,?,?,?,?,?)";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$t_name, $time, $from, $to, $t_no,$day]);
        $sm = "New Transport schedule successfully";
        header("Location: ../transport-add-special-town.php?success=New Transport schedule successfully");
        exit;
	}
